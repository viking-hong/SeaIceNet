...
PALETTE = [[0, 0, 0],[255, 255, 255]]#, [0, 0, 255][255, 255, 255],
...
class SeaIceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(mask.dtype)
        mask_new = torch.ones_like(mask)
        mask_z = torch.zeros_like(mask)
        mask = torch.where(mask <= 100, mask_z, mask_new)
        #mask = torch.where(mask >= 200, mask_new, mask)
        #mask = torch.where(mask > 100, mask_new*2, mask)
        #####################################
